...
router.get('/semanal', auth_1.authenticateToken, async (req, res) => {
    try {
        const tipo = req.query.tipo || 'Culto';
        const result = await (0, database_1.queryAsync)(`SELECT
        rp.dia_semana,
        rp.hora,
...
router.get('/mensual', auth_1.authenticateToken, async (req, res) => {
    try {
        const tipo = req.query.tipo || 'Culto';
...
router.get('/anual', auth_1.authenticateToken, async (req, res) => {
    try {
        const tipo = req.query.tipo || 'Culto';
        const result = await (0, database_1.queryAsync)(`SELECT
